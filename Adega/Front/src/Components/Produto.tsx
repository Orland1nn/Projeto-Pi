...
export default function Produto({ id, imagem, nome, preco, quantidade }: ProdutoProps) {
  const router = useRouter();
  return (
    <div
      key={id}
      onClick={() =>  router.push(`/InterfacePrincipal/Produtos/${id}?nome=${encodeURIComponent(nome)}&preco=${encodeURIComponent(preco)}&imagem=${encodeURIComponent(imagem)}&quantidade=${quantidade}`)
  }
